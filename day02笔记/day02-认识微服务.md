...
- 面向服务：面向服务是说每个服务都要对外暴露Rest风格**（rest风格：http请求，与语言无关）**服务接口API。并不关心服务的技术实现，做到与平台和语言无关，也不限定用什么技术实现，只要提供Rest的接口即可。
- 自治：自治是说服务间互相独立，互不干扰
...
- Http(超文本传输协议)：http其实是一种网络传输协议（请求响应：请求行（request line）、请求头部（header）、空行和请求数据四个部分组成），基于TCP(三次握手连接,四次挥手关闭)，规定了数据传输的格式。现在客户端浏览器与服务端通信基本都是采用Http协议。也可以用来进行远程服务调用。缺点是消息封装臃肿。

  现在热门的Rest风格，就可以通过http协议来实现。
  ***Http与语言技术无关，RPC需要规定的语言技术  ***
...
public class User implements Serializable {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)//主键自增返回主键
    private Long id;
...
**（@EnableDiscoveryClient：内部兼容Eutrka、zookepper等多种的注册中心）**
...
eureka:
  ...
  instance:
    ...
    ip-address: 127.0.0.1 # 指定自己的ip信息，不指定的话会自己寻找启动速度慢
```
