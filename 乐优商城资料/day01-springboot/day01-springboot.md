...
**(classpath实际的路径就是项目结构中的resources，所以就会使用第三种方式)**
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    private Logger logger = LoggerFactory.getLogger(LoginInterceptor.class);//参数为该类的类名
    @Override
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        //不要再写system.out了，显得很low，以后用日志输出
...
**（也可以在main方法的类上写@MapperScan（“包名”）自动给你扫描所有的mapper接口）**
...
**（通用mapper只能解决单表的操作，多表复杂操作还需要自己写）**
